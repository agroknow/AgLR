...
$itemTitle = __('View Template #%s', item('id')) . $itemTitle;
?>
...
<?php if (has_permission($item, 'edit')): ?>
    <p id="edit-item" class="edit-button" style="position:relative;top: -35px;"><?php echo "<a class='edit' href='".uri('templates/edit/'.$item->id)."' onclick='showloader();'>".__('Edit this Template')."</a>"; ?>
    </p>   
    <?php endif; ?>
     <?php
                                        if (has_permission($item, 'edit')): ?>
                                         <?php echo '<p style="position:relative;top: -35px;margin-right:10px;" id="edit-item" class="edit-button"><a href="javascript:void(0);" class="edit" onclick="translatediv(\''.$item->id.'_trans\',\''.$item->id.'\')">'.__('Use Template').'</a><p>'; ?> 
...
//query for all values
            $sql = "SELECT * FROM metadata_record WHERE object_id=" . $item->id . " and object_type='template'";
