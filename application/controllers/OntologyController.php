 ...
 * @copyright Roy Rosenzweig Center for History and New Media, 2007-2010
 * */
class OntologyController extends Omeka_Controller_Action {

    public function getontologyAction() {
        ...
        $params = $this->_getAllParams();
        if ($params['api'] === 'ol-ontology-api') {
            $this->_helper->viewRenderer->renderScript('ontology/transform_owl_to_internal_xml.php');
        } else {
            $this->_helper->viewRenderer->renderScript('index.php');
        }
    }

    public function olontologyAction() {
        $this->getResponse()->setHeader('Content-Type', 'text/xml');
        $this->_helper->viewRenderer->renderScript('ontology/TestRequestOntology.php');
    }
